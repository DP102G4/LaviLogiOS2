...
class QRCodeUserVC: UIViewController, UISearchBarDelegate {
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        if text == "" {
            ...
            searchUsers = users.filter({ (user) -> Bool in
                return user.account!.uppercased().contains(text.uppercased())
            })
            search = true
        }
...
extension QRCodeUserVC: UITableViewDataSource, UITableViewDelegate {
